...
//1 if training is finished
//5 if training does not converge
int TrainingWithDispersionAutomaticInitialization(float sample[]){
    ...
    switch(stateTraining){
        ...
        case Clusters_Comparison :
        {
            ...
            actualNumberOfClusters++;
            LastClusterDetected = actualNumberOfClusters;
            #if COMUSBDEBUG
                PrintClusterUSB(sample, QneuronsInputLayer,
                            actualNumberOfClusters+1);
            #endif
...
int Training(float sample[]){
    ...
    switch(stateTraining){
        case First_Sample :
        {
            for (int i = 0; i < QneuronsInputLayer; i++){
//                if(sample[i] > offSetCluster)
//                    DispersionsInitialValue[i] = sample[i];
//                else
                    DispersionsInitialValue[i] = offSetCluster;
            }
